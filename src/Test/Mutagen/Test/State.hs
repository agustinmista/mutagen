...
import Test.Mutagen.Tracer.Store (TraceStore (..))
...
createInitialState :: forall log. (TraceStore log) => Config -> Property -> IO (State log)
createInitialState cfg (Property gen argsRunner) = do
  ...
  passedTraceLog <- newTraceStore traceNodes
  discardedTraceLog <- newTraceStore traceNodes
